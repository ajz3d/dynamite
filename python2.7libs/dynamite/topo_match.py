...
@P = pointattrib(1, "P", @ptnum, success);"""
    ...
    group_copy = subnet.createNode('groupcopy')
    group_copy.setName('copy_groups')
    group_copy.parm('primgroups').set('*')
    group_copy.parm('pointgroups').set('*')
    group_copy.parm('edgegroups').set('*')
    ...
    group_copy.setInput(0, delete_groups)
    group_copy.setInput(1, subnet.indirectInputs()[0])
    normals.setInput(0, group_copy)
